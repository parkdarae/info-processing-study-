...
def has_image(text):
    indicators = ['figure', 'diagram', 'chart', 'graph', 'table', '그림', '도표', '차트', '표']
...
for i, (answer_pos, answer) in enumerate(answer_positions):
    try:
        ...
        print(f"  [ERROR] 문제 {question_number} 오류: {e}")
        question_number += 1
        continue
