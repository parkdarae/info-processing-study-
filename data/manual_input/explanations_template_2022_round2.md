...
**문제**:
```
다음은 대칭 키 알고리즘에 대한 설명이다.
해당 설명에 맞는 용어를 보기에 골라 작성하시오.
```
...
**문제**:
```
다음은 SOLID 원칙에 관한 내용이다.
괄호안에 알맞는 단어를 보기에서 고르시오.
...
**현재 해설**: (문제18은 소문제 1-3으로 분리됨)

**상태**: ⚠️ 부족 (부족)

**이미지**: images/2022_round2/18.png
...
**문제**:
```
성적은 {학생,학과}에 대해서 ( ) Functional Dependency이다.
```
...
**문제**:
```
성적은 학과만 알아도 식별이 가능하므로, 이 경우에는 성적 속성은 기본키에 ( ) Functional Dependency이다.
```
...
**문제**:
```
릴레이션에서 X, Y, Z라는 3 개의 속성이 있을 때 X→Y, Y→Z 이란 종속 관계가 있을 경우, X→Z가 성립될 경우
```
